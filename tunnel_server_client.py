            ...
            try:
                ...
                if e.args[0] == errno.EAGAIN or e.args[0] == errno.EWOULDBLOCK:
                    ...
                    if shouldSendFakePacket():
                        ...
                        b_fake_length = l_fake.to_bytes(2, byteorder='big')
                        bb4 = bytearray(b'\x01')
                        bb5 = bytearray(b_fake_length)
                        bb6 = bytearray(fake_packet)
                        buf = bytes(bb4+bb5+bb6)
                    else:
                        sleep(0.1)
                        continue
                else:
