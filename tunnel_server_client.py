    ...
    def srvThreadTapReadFunction(tap, conn_list):
        ...
        while True:
            try:
                ...
                l = len(packet)
                l_padd = len(padd)
                #total_length = l + l_padd
                #b_total_length = total_length.to_bytes(2, byteorder='big')
                b_packet_length = l.to_bytes(
                    2, byteorder='big')
                b_padding_length = l_padd.to_bytes(
                    2, byteorder='big')
                bb1 = bytearray(b'\x00')
                #bb2 = bytearray(b_total_length)
                ...
                if e.args[0] == errno.EAGAIN or e.args[0] == errno.EWOULDBLOCK:
                    ...
                    if shouldSendFakePacket():
                        ...
                        fake_padd = padding(fake_packet)
                        l_fake = len(fake_packet)
                        l_fake_padd = len(fake_padd)
                        total_fake_length = l_fake + l_fake_padd
                        b_total_fake_length = total_fake_length.to_bytes(
                            2, byteorder='big')
                        ...
                        #bb7 = bytearray(b_total_fake_length)
