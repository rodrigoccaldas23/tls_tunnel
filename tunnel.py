...
class TLSTunnel():
    ...
    def initialize(self):
        ...
        parser.add_argument('--priv_net_addr',
                            dest='priv_net_addr', type=str, default='192.168.1.3')
        ...
        parser.add_argument('--server_sni_hostname',
                            dest='server_sni_hostname', default='client')
        ...
        if self.args.mode == 'client':
            ...
            self.server_cert = '/home/rodrigotese/tunelv2/certs/server.crt'
            self.client_cert = '/home/rodrigotese/tunelv2/certs/client.crt'
            self.client_key = '/home/rodrigotese/tunelv2/certs/client.key'
            ...
            try:
               ...
               # print("passou o bb123")
                ...
                if e.args[0] == errno.EAGAIN or e.args[0] == errno.EWOULDBLOCK:
                    ...
                    print(e)
                    raise
#            print("antes do conn_list")
 #           print("len conn_list =" + str(len(conn_list)))
    ...
    def srvThreadTapWrite(self, clientsocket, addr, clienttap, conn_list, buckets):
        ...
        while new_conn['kill_signal'] == False:
            try:
                ...
                if ready[0]:
                    print("antes do get_pacj_tls")
                    data = self.get_packet_from_tls(conn)
                    print("encontrou data")
...
if tls.args.mode == 'client':
    ...
    print("start tap reading and sending to clients via TLS")
    tap_thread = threading.Thread(target=TLSTunnel.srvThreadTapReadFunction, args=(
        tls.tap, tls.active_connections,))
    tap_thread.daemon = True
    tap_thread.start()

    print("start server thread waiting for new clients and spawn new tap read from client and write to tap")
